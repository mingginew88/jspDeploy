...
public class LprodService implements LprodServiceInf {
	...
	*/
	...
	public Map<String, Object> getPageLprodList(Map<String, Integer> map) {
		...
		mapStrBuff.append("<li><a href=\"/lprodList?page="+ prevPage + "&pageSize=" + pageSize +"\" aria-label=\"Previous\"><span aria-hidden=\"true\">&laquo;</span></a></li>") ;
		...
		for(int i = 1; i <= cnt; i++) {
			...
			mapStrBuff.append("<li" + active + "><a href=\"/lprodList?page="+ i + "&pageSize=" + pageSize + "\">" + i + "</a></li>") ;
		}
		mapStrBuff.append("<li><a href=\"/lprodList?page="+ nextPage + "&pageSize=" + pageSize + "\" aria-label=\"Next\"><span aria-hidden=\"true\">&raquo;</span></a></li>");
