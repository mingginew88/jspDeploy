...
public class StudentService implements StudentServiceInf {
	...
	*/
	...
	public Map<String, Object> getStudentList(Map<String, Integer> map) {
		...
		//학생 페이지 리스트 조회
		List<StudentVo> pageList = studentDao.getStudentList(map);
	...
	*/
	private String makePageNavi(int page, int pageSize, int totCnt) {
		...
		pageNaviStr.append("<li><a href=\"/showBoardList?page="+ prevPage +"&pageSize=" + pageSize + "\" aria-label=\"Previous\"><span aria-hidden=\"true\">&laquo;</span></a></li>");
		...
		for(int i = 1; i <= cnt; i++) {
			...
			pageNaviStr.append("<li"+activeClass+"><a href=\"/showBoardList?page="+ i +"&pageSize=" + pageSize + "\">" + i + "</a></li>");
		}
		pageNaviStr.append("<li><a href=\"/showBoardList?page="+ nextPage +"&pageSize=" + pageSize + "\" aria-label=\"Next\"><span aria-hidden=\"true\">&raquo;</span></a></li>");
